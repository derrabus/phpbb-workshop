...
if ($submit) {
    ...
    if ($user_logged_in) {
        ...
        if ($userdata[user_id] != $posterdata[user_id]) {
            if (1 == $userdata[user_level]) {
                $die = 1;
            } elseif (2 == $userdata[user_level] && !is_moderator($forum_id, $userdata[user_id], $db)) {
    ...
    if (!$delete) {
        ...
        if (0 == get_total_posts($topic_id, $db, 'topic')) {
...
<?php
    ...
    if ($user_logged_in) {
        if ($userdata[user_level] <= 2) {
            if (2 == $userdata[user_level] && !is_moderator($forum, $userdata[user_id], $db)) {
