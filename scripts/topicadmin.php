...
if ($HTTP_POST_VARS['submit'] || (1 == $user_logged_in && 'viewip' == $mode)) {
    if (1 != $user_logged_in && ($mod_data[user_password] != md5($passwd))) {
        error_die('Error - You did not enter the correct password, please go back and try again.');
    }
...
<?php
    ...
    while ($row = $r->fetch(\PDO::FETCH_BOTH)) {
        ...
        echo '	<TD><A HREF="bb_profile.php?mode=view&user='.$row[user_id].'">'.$row[username]."</A></TD>\n";
        echo '	<TD>'.$row[postcount]." posts</TD>\n";
        echo "</TR>\n";
    }
?>
